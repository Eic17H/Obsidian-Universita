...
Diciamo che un positivo o un negativo sono soddisfatti quando la proprietà potrebbe valere o potrebbe non valere, e sono asseriti quando si dimostra che vale o che non vale. I primi si indicano con `P` ed `N`, i secondi con `P!` ed `N!`. `P` può essere un falso positivo ed `N` può essere un falso negativo. Si può avere un falso negativo quando c'è una chiamata esterna, quindi il tool non può sapere se il contratto chiamato fa qualcosa che non va bene per la proprietà.

Si possono usare due solver: Z3 ed Eldarica. Ci sono cose che uno può dimostrare e l'altro no.
