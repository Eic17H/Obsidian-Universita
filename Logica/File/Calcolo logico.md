...
Vediamo il nostro linguaggio $\mathcal{L}_0$.
...
* $\mathcal V (α \rightarrow β) = 1$ se e solo se $\mathcal V (α) = 0$, o $\mathcal V (β) = 1$, o entrambi
...
$α \in For(\L_0) \land Γ \subseteq For(\L_0)$ ($α$ è una formula, $Γ$ è un insieme di formule), allora diciamo che $α$ è conseguenza logica in HK (momentaneamente usiamo HK come sinonimo di logica classica) di $Γ$, e scriviamo in simboli $Γ \models_{HK} α$ quando per ogni valutazione $\mathcal V$ in $\mathcal L_0$ se $\mathcal V(γ)=1$ per ogni $γ$ in $Γ$, allora $\mathcal V(α)=1$ (cioè non ha controesempi).
...
Vediamo meglio le tautologie e gli schemi validi. $p, p \rightarrow q \models_{HK} q$. $p$ deve essere $1$, e $p \rightarrow q$ deve essere $1$, se $q$ fosse $0$ quello sarebbe impossibile quindi $q=1$, QED.
