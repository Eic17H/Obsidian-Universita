...
1) Moltiplicare entrambi i "lati" per lo stesso scalare. ${EQ}_i \leftarrow a{EQ}_i$
2) Posso rimpiazzare un'equazione con la somma tra due equazioni (metodo di riduzione). $EQ_i \leftarrow EQ_i + EQ_j$
3) Posso scambiare due equazioni. $EQ_i \leftrightarrow EQ_j$

**Non ottengo la stessa matrice. Il sistema associato ha la stessa soluzione.**

Se ho $3x-y$ in una e $12x-y$ nell'altra, cosa posso fare? L'algoritmo di Gauss fa $riga_i \leftarrow riga_i - m_{i1} \cdot riga_1$, ed $m_{i1} = \dfrac{a_{i1}}{a_{11}}$, cioè moltiplichi $3x-y$ per $\dfrac{12}{3}$.
...
4\\
\end{bmatrix}$$
Allora. Cosa posso fare? $m_{21}=\dfrac{1}{3}$, $m_{31} = \dfrac{4}{3}$. E faccio, $riga_2 \leftarrow riga_2 - m_{21} \cdot riga_1$.$$\begin{bmatrix}1&2&4\end{bmatrix} - \dfrac{1}{3}\begin{bmatrix}3&2&1\end{bmatrix}=\begin{bmatrix}1&2&4\end{bmatrix}-\begin{bmatrix}1&\dfrac{2}{3}&\dfrac{1}{3}\end{bmatrix} = \begin{bmatrix} 0&\dfrac{4}{3}&\dfrac{11}{3}\end{bmatrix}$$
