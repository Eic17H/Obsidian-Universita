...
cssclasses:
  - so
---
Vediamo un po' di comandi del terminale relativi ai file.
...
* 3 relativi al proprietario di cui sopra.
* 3 relativi al gruppo di cui sopra.
* 3 relativi agli altri utenti.
Quindi c'è `r`ead, `w`rite, `x` per ciascuno.
...
Una brutta pratica è di dare `rwx` a tutti. Veramente sbagliato. Ti semplifica temporaneamente il lavoro, ma un utente malintenzionato o sbadato causa problemi.
...
Simbolico:
* Chi: `U`ser, `G`roup, `O`ther, `A`ll (default `a`)
...
Puoi cambiare l'owner e il gruppo: `chown` e `chgrp`.
